...
<@default title="${tag.name} - ${options.blog_title!'vno'}" keywords="${options.seo_keywords!}" desc="${options.seo_description!}" canonical="${ctx!}/tags/${tag.slugName}">
    <div class="main-post-list hidden">
        <ol class="post-list">
            <#list posts.content as post>
            <li>
                <h2 class="post-list__post-title post-title"><a href="${ctx!}/archives/${post.url}" title="访问 ${post.title}">${post.title}</a></h2>
        ...
        <@nav url = "${ctx!}/tags/${tag.slugName}/"></@nav>
    </div>
</@default>